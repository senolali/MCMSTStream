...
class MCMSTStream:
    ...
    dense_mc_promotion : bool, default=False
        If True, a group of micro-clusters (including a single micro-cluster)
        also qualifies as a macro-cluster when its total number of data points
        is at least ``n_micro * N``, even if it contains fewer than ``n_micro``
        micro-clusters. Default False reproduces the behavior of the original
        paper (Erdinç, Kaya & Şenol, 2024).
    ...
    def __init__(self, W=200, n_micro=2, N=2, r=0.1, random_state=None, keep_history=False,
        ...
        self.dense_mc_promotion = dense_mc_promotion
    ...
    def _component_qualifies_as_macro(self, mc_list):
        """Check if a group of micro-clusters qualifies as a macro-cluster.

        A group qualifies if it contains at least ``n_micro`` micro-clusters.
        If ``dense_mc_promotion=True``, a group also qualifies when its total
        number of data points is at least ``n_micro * N`` (a single very dense
        micro-cluster can thus form a macro-cluster on its own). This extends
        the original paper's Algorithms 4, 8, and 10.
        """
        if len(mc_list) >= self.n_micro:
            return True
        if not self.dense_mc_promotion:
            return False
        total_points = sum(mc['n_points'] for mc in mc_list)
        return total_points >= self.n_micro * self.N
    ...
    def _define_macro_clusters(self):
        ...
        for i in range(len(unassigned_mcs)):
            ...
            component_mcs = [unassigned_mcs[idx] for idx in current_cluster]
            if self._component_qualifies_as_macro(component_mcs):
    ...
    def _update_macro_clusters(self):
        ...
        for macro in self.macro_clusters:
            ...
            if not self._component_qualifies_as_macro(mc_list):
                macro['active'] = False
                continue
            ...
            if len(mc_centers) > 1:
                ...
                for component in new_mc_groups:
                    component_mcs = [mc_list[idx] for idx in component]
                    if self._component_qualifies_as_macro(component_mcs):
    ...
    def get_params(self, deep=True):
        ...
        return {
            ...
            'dense_mc_promotion': self.dense_mc_promotion
        }
